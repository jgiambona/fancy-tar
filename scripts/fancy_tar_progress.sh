...
for arg in "$@"; do
  case "$arg" in
    ...
    --self-test)
      ...
      tmpdir=$(mktemp -d)
      echo "test content" > "$tmpdir/sample.txt"
      "$0" "$tmpdir/sample.txt" -o "$tmpdir/test.tar.gz" >/dev/null
...
# Prompt for password if --zip --password is set without value
if [[ "$use_zip" = true && -n "$password" && "$password" == "--zip" ]]; then
  confirm_password
fi
...
if [ "$use_zip" = true ]; then
  ...
  echo "✅ ZIP archive created: $output"
  exit 0
fi
...
# If not zip, fallback to tar/tar.gz creation
extension=".tar.gz"
[ "$gzip" = false ] && extension=".tar"
[[ "$output" != *"$extension" ]] && output="${output}${extension}"
...
# Clean up incomplete archive if it doesn't exist or failed
if [ ! -f "$output" ]; then
  echo "❌ Archive creation failed or was interrupted."
